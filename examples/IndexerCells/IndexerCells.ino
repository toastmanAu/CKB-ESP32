/*
 * IndexerCells.ino
 * CKB-ESP32 example — query all live cells for an address.
 * Shows pagination, capacity totals, and type script detection.
 *
...
// Address to query
const char* TARGET_ADDR = "ckb1q...your-address-here";
...
void setup() {
    ...
    do {
        ...
        for (uint8_t i = 0; i < result.count; i++) {
            const CKBIndexerCell& c = result.cells[i];
            const char* typeLabel = identifyTypeScript(c.output.type);

            Serial.printf("  [%u] %s\n",
                totalCells + i, CKBClient::formatCKB(c.output.capacity).c_str());
